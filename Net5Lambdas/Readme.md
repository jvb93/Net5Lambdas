...
WORKDIR /src
COPY ["Net5Lambdas.csproj", "Net5Lambdas/"]
RUN dotnet restore "Net5Lambdas/Net5Lambdas.csproj"

WORKDIR "/src/Net5Lambdas"
COPY . .
RUN dotnet build "Net5Lambdas.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "Net5Lambdas.csproj" \
...
Execute unit tests
```
    cd "Net5Lambdas/test/Net5Lambdas.Tests"
    dotnet test
```

Deploy application
```
    cd "Net5Lambdas/src/Net5Lambdas"
